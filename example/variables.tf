...
// File Locations
variable "s3_routing_rules_input" {
  description = "Json document that describes the routing rules for the s3 bucket. Set to null if there are none."
  type        = string
  default     = "./policies/routing_rules.json"
}

variable "iam_assume_role_policy_input" {
  description = "Role policy definition for assuming a role capable of enabling replication in another region."
  type        = string
  default     = "./policies/iam_assume_role.json"
}
